...
md"""# Syntactic search for functional alignment"""

# ╔═╡ fb90ae06-1c3b-4b85-b9d9-fd577ea4b8c1
md"""*Number of passages to display* $(@bind maxdisplay Slider(1:50, default=50, show_value = true))"""
...
# ╔═╡ 223be57f-04e6-40cc-9060-9f4e43147a6b
md"""> ## Syntactic selection"""
...
function displayalignment(u, c)
	...
	greeksept = filter(c.passages) do p
		dropversion(p.urn) == reduced && versionid(p.urn) == "septuagint"
	end
	vulg = filter(c.passages) do p
		dropversion(p.urn) == reduced && versionid(p.urn) == "vulgate"
...
# ╔═╡ 62a5f32e-963d-4274-ac0a-f59bdc8c2415
verbcandidates = filter(greekparsedtokens.analyses) do a
    frmslist = a.analyses .|> greekForm
    verbs = filter(frmslist) do frm
...
# ╔═╡ 17ff3460-e57a-4977-99aa-da57c685553d
md"""## $(length(verbcandidates)) possible verb forms found"""

# ╔═╡ 6be6c344-d4b1-49b3-b3c8-7ebf22fceda1
begin
	typelists = []
	for v in verbcandidates
		atypes = map(v.analyses) do a
			a |> greekForm |> typeof
		end
		push!(typelists, unique(atypes))
	end
	typeinstances = typelists |> Iterators.flatten |> collect
end

# ╔═╡ 3f665cdb-4c5b-44ed-8292-272ad83b4e8f
begin
typecounts = countmap(typeinstances) |> OrderedDict
sort!(typecounts, byvalue=true, rev=true)
end

# ╔═╡ 512952af-9b77-4d59-923a-b698394df6f8
begin
	menu = [ 0 => ""]
	for (i, k) in enumerate(keys(typecounts))
		typelabel = replace(string(k),"Kanones.GMF" => "") 
		msg = string(typelabel, ": ", typecounts[k], " passages")
		pr = i => msg
		push!(menu, pr)
	end
	menu
...
# ╔═╡ c9bb8f30-4d7a-48eb-bf51-4712843f2e79
@bind posidx Select(menu)

# ╔═╡ 98bf3b05-a409-4990-b109-cb5459ba440a
pos = if posidx > 0
	collect(keys(typecounts))[posidx]
else
	nothing
end

# ╔═╡ 8b95ac44-a370-4d96-b9d6-210ca2e9dc95
poscount = isnothing(pos) ? 0 : typecounts[pos]

# ╔═╡ 86419277-5e49-4ee3-89f1-6357c9ad98cc
slivers = convert(Int, round(poscount / maxdisplay))

# ╔═╡ 04c6891d-4bcf-4886-96b4-5415671fff13
md"""*Slice of **$(maxdisplay)** to view*: $(@bind sliver Slider(1:slivers))"""

# ╔═╡ cad8f058-31b0-43ed-8ed7-bbfbd79d89c8
sliver

# ╔═╡ 60b8cbc4-5238-4d3c-879d-f9ee047203ce
posidx

# ╔═╡ c9211e07-f214-4fca-a5a6-a42d5bc8b6b5
selectedtype = collect(keys(typecounts))[posidx]

# ╔═╡ 86024322-dcca-4102-b001-51e00b0b1e90
function hastype(v, t)
	foundit = false
	for a in v
		if greekForm(a) isa selectedtype
			foundit = true
		end
	end
	foundit
...
# ╔═╡ d9083c4a-5a10-45a9-96f7-541b9af7f8ff
passagelist = filter(verbcandidates) do v
	hastype(v.analyses, selectedtype)
end
	

# ╔═╡ d674bfef-d23b-413f-bcab-e01b19ecb61b
begin
	i1 = (sliver - 1) * maxdisplay + 1
	i2 = sliver * maxdisplay + 1
	selectedpsgs = passagelist[i1:i2]
end

# ╔═╡ 4d30b42d-215a-4983-81be-4657d818861d
ulist = [p.ctoken.passage.urn for p in selectedpsgs]

# ╔═╡ 649f3639-615f-43b6-9a2f-6225359d2f01
join([displayalignment(u, corpus) for u in ulist], "\n\n") |> Markdown.parse
...
manifest_format = "2.0"
project_hash = "da8c8c509d8b6e8842a2409753203ab55133c104"
