...
# ╔═╡ 5250d1c4-0135-43c2-bdbc-6ccb4da55c19
begin
...
# ╔═╡ f6d7d1c4-9486-41cb-9e43-b4a27f133965
md"""*Notebook version*: **1.1.1** *See version info* $(@bind versioninfo CheckBox())"""

# ╔═╡ d67a7a9b-fae4-452c-8981-061d10ffcd3a
if versioninfo
	md"""Version history:

- **1.1.1**: Works with new URNs
- **1.1.0**: Add embedded version of image to results	
- **1.0.0**: initial release
"""
end

# ╔═╡ 1fe3a7f6-0004-4212-873d-c3cbb3a0c95d
@bind reloaddata Button("Reload indexing data")

# ╔═╡ 436813f6-bb93-4e01-b900-d7d93d2d9db8
md"""> *Image URNs are linked to the Homer Multitext project's Image Citation Tool.*"""
...
# ╔═╡ 4c8ad88d-8044-4e20-b0dd-993cda069e48
md"""
---
"""

# ╔═╡ ba7a5038-9b9d-4bf8-b69c-5195c86134ef
md"""> Display linked thumbnail."""
...
function chaptersforbook(data, bookid)
	bookpassages = filter(data) do pr
		workid(pr.ref) == lowercase(bookid)
	end
	map(bookpassages) do psg
		collapsePassageTo(psg.ref, 1) |> passagecomponent
...
function versesforchapter(data, bk, chptr)
	chapterpassages = filter(data) do pr
		psgchapter = collapsePassageTo(pr.ref, 1) |> passagecomponent
		workid(pr.ref) == lowercase(bk) && psgchapter == chptr
	end
	map(chapterpassages) do pr
		passagecomponent(pr.ref)
	end |> unique
end
...
# ╔═╡ 1516fb23-afc6-458a-804e-25bb0d5a2dbd
ictbase = "https://www.homermultitext.org/ict2/?"

# ╔═╡ e8b585ad-07c4-40fd-b45a-4b3a41409a6e
md"""> Load data"""

# ╔═╡ 8ce2b58b-8dd2-4976-96e5-d1894b4fd56a
srcurl = "https://raw.githubusercontent.com/neelsmith/compnov/main/corpus/compnov.cex"

# ╔═╡ 11ccd5d7-150f-4e4f-ae37-3d1ecc1fae4a
corpus = fromcex(srcurl, CitableTextCorpus, UrlReader)

# ╔═╡ 56ee1ad3-4f65-44a3-abc8-ce962a651bdd
vulgate = filter(corpus.passages) do psg
	versionid(psg.urn) == "vulgate"
end |> CitableTextCorpus

# ╔═╡ 6de75d10-583d-4d08-aaee-04a1861981c6
md"""> Indexing data"""

# ╔═╡ a3c57a32-211d-45f1-86bc-03792069b7bd
repo = dirname(pwd()) |> dirname

# ╔═╡ 71e6abdb-9d45-4205-acde-98929beaef96
rawdata = begin
	reloaddata
	idxfile = joinpath(repo, "indexing", "vulgate-to-image-vol1.cex")
	filter(readlines(idxfile)[2:end]) do ln
		!isempty(ln)
	end
end

# ╔═╡ 0e97e5b5-6069-43ac-9147-c83a8956de0b
joinpath(repo, "indexing", "vulgate-to-image-vol1.cex")

# ╔═╡ e6a1cdc0-5715-4d39-b8d0-425dd2ff7627
"""Retrieve range of passages given by a range URN from a given corpus."""
function retrieverange(rngurn, c)
	startpoint = findfirst(c.passages) do p
		passagecomponent(p.urn) == range_begin(rngurn)
	end
	endpoint = findfirst(c.passages) do p
		passagecomponent(p.urn) == range_end(rngurn)
	end
	c.passages[startpoint:endpoint]
end

# ╔═╡ 2fd0ffcf-5a28-4bb4-a93e-d64ca3012c69
begin
	data = []
	for ln in rawdata
		cols = split(ln, "|")
		rng = CtsUrn(cols[1])
		img = cols[2]
		psglist =  retrieverange(rng, vulgate)
		for psg in psglist
			push!(data, (ref = psg.urn, image = img))
		end
	end
	data
end
...
bklist = map(data) do pr
	titlecase(workid(pr.ref))
end |> unique

# ╔═╡ 8dad12da-ca87-4e29-8d29-149ba08110dc
md"""*Book*: $(@bind bk Select(bklist))"""

# ╔═╡ 0a76d187-b061-42b5-a731-adad616d91ca
md"""*Chapter*: $(@bind chap Select(chaptersforbook(data, bk)))"""

# ╔═╡ 05daa7bf-bb17-48ad-bcd0-0f22294cd4ea
md"""*Verse*: $(@bind verse Select(versesforchapter(data, bk, chap)))"""

# ╔═╡ 6011d8dc-cbd9-459f-a74e-f8ee5249f50f
queryurn = CtsUrn("urn:cts:compnov:bible.genesis.vulgate:$(verse)")

# ╔═╡ 0bd64110-0dd9-43a1-bdf4-1794c979a950
indexpairs = filter(data) do pr
	pr.ref == queryurn
end

# ╔═╡ 461ba932-c3bf-46c4-864e-88e951ab3674
i1 = indexpairs[1].image |> Cite2Urn

# ╔═╡ 529cdb8a-aa7d-4f2b-9918-eb4befd70d25
begin


	mdlines = []
	if isempty(indexpairs)
		push!(mdlines, "No images indexed for *$(bk)*, $(verse)")
	elseif length(indexpairs) == 1
		push!(mdlines, "One image indexed for *$(bk)*, $(verse)")
		
	else
		push!(mdlines, "**$(length(indexpairs)) images** indexed for *$(bk)*, $(verse):")
	end

	for pr in indexpairs
		lnk = string(ictbase, "urn=", pr.image)
		ln = string("- [", pr.image, "](", lnk, ")")
		push!(mdlines, ln)
	end
...
# ╔═╡ c04a518c-577e-48e8-bbd6-fd46845c8ce6
md"""> Image services"""

# ╔═╡ 3308dd7e-6f13-47c3-b8fc-fe9406c0b378
baseurl = "http://www.homermultitext.org/iipsrv"

# ╔═╡ 8f967a7b-67d4-4c2c-aefb-6fe870fa7d27
root = "/project/homer/pyramidal/deepzoom"

# ╔═╡ e61c5a26-0e12-4493-ac03-dc044f275ca8
service = IIIFservice(baseurl, root)

# ╔═╡ 16c04f57-8f4d-4687-923c-6f9ea7c1c47f
begin
	img_md_lines = []
	for pr in indexpairs
		push!(img_md_lines, markdownImage(Cite2Urn(pr.image), service; w = 100))
...
manifest_format = "2.0"
project_hash = "53ad70c0e805409721f10e7f9e946871d1a0a931"
